(function () {
  ...
  $('#marketplace-type-select').on('select2:opening', function (e) {
    $('#msform .form-group-custom.focused').removeClass('focused');
  });
  ...
  function formatMarketplaceTypes(marketplaceType) {
    if (!marketplaceType.id) {
      return marketplaceType.text;
    }
    var $marketplaceType = $(
      '<span>' + marketplaceType.text + "</span>"
    );
  ...
  $("#marketplace-type-select").prepend('<option></option>').select2({
    templateResult: formatMarketplaceTypes,
